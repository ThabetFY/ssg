    ...
    for entry in os.listdir(dir_path_content):
        ...
        if os.path.isdir(entry_path):
            generate_pages_recursive(
                basepath, entry_path, template_path, entry_dest_path
            )
        else:
            entry_dest_path = Path(entry_dest_path).with_suffix(".html")
            generate_page(basepath, entry_path, template_path, entry_dest_path)


def generate_page(basepath: str, from_path: str, template_path: str, dest_path: Path):
